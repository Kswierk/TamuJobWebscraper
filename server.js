...
async function getYearDataForMajor(context, request, sem) {
    ...
    console.log(dom.window.document.querySelector("#resultstbl > tbody"));
...
async function getMajorData(request, browser) {
    ...
    while (semesters.length > 0) {
        ...
        console.log(temp);
        try {
            (await Promise.all(temp.map(el => getYearDataForMajor(context, request, el)))).forEach(e => e.length != 0 ? majorData.push(...e) : "");
        } catch (e) {
            throw (e);
        }
